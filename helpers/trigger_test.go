...
func TestTriggerGetByKeySuccess(t *testing.T) {
	...
	mock.ExpectQuery("SELECT id, contentId, key, params FROM trigger").
...
func TestTriggerInsertSuccess(t *testing.T) {
	...
	mock.ExpectPrepare("INSERT INTO trigger").
		ExpectExec().
...
func TestTriggerUpdateSuccess(t *testing.T) {
	...
	mock.ExpectPrepare("UPDATE trigger").
		ExpectExec().
...
func TestTriggerDeleteSuccess(t *testing.T) {
	...
	mock.ExpectPrepare("DELETE FROM trigger").
		ExpectExec().
		WithArgs(trigger.Key).
		WillReturnResult(sqlmock.NewResult(1, 1))
